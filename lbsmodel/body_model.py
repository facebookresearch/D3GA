...
class LinearBlendSkinning(nn.Module):
    ...
    def compute_root_rigid_transform(self, poses: th.Tensor) -> Tuple[th.Tensor, th.Tensor]:
        ...
        scales = torch.zeros(
            (poses.shape[0], self.param_transform.nr_total_params - poses.shape[1]),
            dtype=poses.dtype,
            device=poses.device,
        )
...
def compute_root_transform_cuda(lbs_fn, poses, verts=None):
    ...
    scales = torch.zeros(
        (poses.shape[0], lbs_fn.param_transform.nr_total_params - poses.shape[1]),
        dtype=poses.dtype,
        device=poses.device,
    )

    # NOTE: scales are zero (!)
    _, _, _, state_t, state_r, state_s = lbs_fn(poses, scales)
...
class LBSModule(nn.Module):
    ...
    def pose(self, motion, template: Optional[th.Tensor] = None):
        scale = self.lbs_scale.expand(motion.shape[0], -1)

        if template is None:
            template = self.lbs_template_verts.expand(motion.shape[0], -1, -1)

        return self.lbs_fn(motion, scales=scale, verts_unposed=template) * self.global_scaling
