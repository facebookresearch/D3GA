...
class Quaternion:
    ...
    def fromXYZ(angles):
        ...
        them.
        """
        rc = th.cos(angles * th.tensor([-0.5, 0.5, 0.5], dtype=angles.dtype, device=angles.device))
        rs = th.sin(angles * th.tensor([-0.5, 0.5, 0.5], dtype=angles.dtype, device=angles.device))
    ...
    def batchInvert(q):
        ...
        """
        return q * th.tensor([-1.0, -1.0, -1.0, 1.0], dtype=q.dtype, device=q.device) * (th.reciprocal(th.sum(q * q, dim=2).unsqueeze(2)))
...
class RodriguesVecBatch(nn.Module):
    ...
    def forward(self, v0, v1):  # assuming v0 and v1 are already normalized, compute matrix aligning v0 to v1
        nbat = v0.size(0)
